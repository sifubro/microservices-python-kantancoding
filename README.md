...
4. The `video_to_mp3`  converter service will consume messages from the queue. It will then
  4a. get the ID of the video from the message
  4b. pull that video from mongoDB
  4c. convert to mp3
  4d. store the mp3 on mongoDB
  4e. put a new message on the Queue to be consumed by the notification service that says "the conversion job is done"
